...
def solve(tool, smt2Filename, SOLVED_PROBLEM, root, timeout, max_memory, TOOL_RESULT, flags):
	...
	# extract weaking time from iErr:
	try:
		wtimeRegex = re.search("wtime (\d+\.\d+) wtime", iErr.strip())
		result[WEAKENING_TIME] = eval(wtimeRegex.group(1))
	except Exception:
		result[WEAKENING_TIME] = 0
...
def run(tool, directory, timeout, resultFile, SOLVED_PROBLEM, max_memory=4000000, flags=""):
	...
	HEADERS = [PROBLEM, RESULT, CPU_TIME, TIME, WEAKENING_TIME, TOOL_RESULT]
